...
def classify_message(API_BASE, MODEL, API_KEY, message):
    System_Prompt = """Classify IT chat messages for a weekly newsletter. Output exactly "1" (relevant) or "0" (not relevant). No other text.
...
def classify_ticket(API_BASE, MODEL, API_KEY, message):
    System_Prompt = """Classify IT chat messages. Output exactly "1" (is a ticket) or "0" (not a ticket). No other text.
...
def create_weekly_newsletter(API_BASE, MODEL, API_KEY, jabber_json, sn_json, profession_json):
    Newsletter_Prompt = """You write "The IT Wire," a weekly internal recap for the URI IT team. Turn the week's chat announcements, Jabber messages, and ServiceNow tickets into a brief, scannable summary.
    # Voice
    Direct. Plain. A little dry. Like a sharp coworker writing a recap, not a corporate memo. Short sentences. Active verbs. No filler ("we wanted to share that..."), no jargon ("synergize," "leverage"), no emojis, no jokes.
